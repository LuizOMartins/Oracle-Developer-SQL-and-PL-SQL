...
-- evidencia de erro	
	...
	-- OPERADOR HAVING
	select cod_uf,estado, count(cod_mun) as qtd 
	from senso
	where ano='2014'
	group by cod_uf,estado having count(cod_mun)>500;
    
    select cod_uf,estado, count(*) as qtd -- seleciona todas as linhas que foram selecionadas
	from senso
	where ano='2014'
	group by cod_uf,estado having count(cod_mun)>500;
    
    select *from SENSO where cod_uf=31 and ano='2010';
	...
	where ano='2014'
	group by cod_uf,estado having count(cod_mun)<=500
    order by 3 desc ;-- terceiro campo com order
	...
	select cod_uf,estado,count(cod_mun)Quantidade,
    sum(populacao)
	from senso
	where ano='2014'
	group by cod_uf,estado having sum(populacao)<5000000;
